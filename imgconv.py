...
#def savedwt(dwt, idx):
#    approx, details  = dwt
#    savearray(approx, "wc{0}approx.png".format(idx), "L")
#    idx+=1
#    #savearray(approx, "wc{0}approx.png".format(idx), "L")
#    for j, detail in enumerate(details):
#        print "DTL:",idx,len(detail)
#        savearray(detail, "wc{0}:{1}detail.png".format(idx,j), "L")
#        idx+=1
#
#def saveswt(swt):
#    for idx, level in enumerate(swt):
#        savedwt(level, idx)
...
#
  #rgb = dot(yuv, A.T)
  #return rgb.clip(0, 255).astype('uint8')A
 #= array([[1.164,     0.,  1.793],
           #[1.164, -0.213, -0.533],
           #[1.164,  2.112,     0.]])
